...
class DenseNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out_2 = F.relu(features, inplace=True)
        out_2 = F.adaptive_avg_pool3d(out_2, (1, 1,1))
        out_2 = torch.flatten(out_2, 1)
        out_2 = self.regressor(out_2)
        out = (out_1+out_2)/2
        return out
...
def densenet_cam(pretrained: bool = False, progress: bool = True, **kwargs: Any):
    return _densenet('densenet_cam', 36, (4, 6,12, 12), 64, pretrained, progress,
                    **kwargs)
